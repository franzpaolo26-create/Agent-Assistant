...
function heuristicTier(text, hasMedia) {
  ...
  if (TIER3_KEYWORDS.some(kw => lower.includes(kw)))      return 3;
  if (TIER2_KEYWORDS.some(kw => lower.includes(kw)))      return 2;
  if (TIER1_PATTERNS.some(rx => rx.test(lower.trim())))   return 1;
  if (text.trim().split(/\s+/).length <= 8)               return 1; // Short message → simple
